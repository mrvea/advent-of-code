...
func main() {
	...
	var maxSleepDurationGuardID string
	var maxDuration time.Duration
	guardsSleepDuration := map[string]time.Duration{}
	...
	for !l.Empty() {
		...
		case "wakes":
			asleep.To = li.T
			guardsSleepDuration[guardID] += asleep.Duration()
			if d := guardsSleepDuration[guardID]; d > maxDuration {
				maxDuration = d
				maxSleepDurationGuardID = guardID
			}
	...
	fmt.Println(maxSleepDurationGuardID)
	fmt.Println(maxDuration)
	r := make([]int, 60)
	maxMinute := -1
	maxCount := 0
	for _, s := range asleeps[maxSleepDurationGuardID] {
	...
	id, err := strconv.Atoi(maxSleepDurationGuardID)
	if err != nil {
		log.Fatalln(err)
	}
	fmt.Printf("ans: %d\n", id*maxMinute)
